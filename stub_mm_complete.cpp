...
// =============================================================================
// Minimal C++ ABI support (for libiatlasservice.so)
// This library has no exported C++ classes but needs basic C++ runtime
// =============================================================================
...
// =============================================================================
// libmmlistservice.so classes (THE MISSING SYMBOL)
//
// This was previously assumed to have no exported C++ symbols, but the
// mmlistservice executable directly instantiates android::MMListService,
// so the concrete class (and its interface chain) must be exported here.
// _ZN7android13MMListServiceC1Ev / C2Ev
//
// NOTE: this class is baked into every .so built from this source file
// (per the --soname trick), but that's harmless — the other targets
// (libmmlistparser.so, libiatlasservice.so, libopluscfgpolicy.so) simply
// won't have anything calling these extra symbols.
// =============================================================================
...
class MMListService : public BnMMListService {
public:
    MMListService();
    virtual ~MMListService();
};

MMListService::MMListService()  {}
MMListService::~MMListService() {}
